...
struct Instruction {
  ...
  union {
    struct /* AllocTensor Operands */ {
      ...
      uint32_t ndim;
      /*! \brief The dimensions of the shape. */
      uint64_t* shape;
   ...
   */
  static Instruction AllocTensor(std::vector<uint64_t> shape, DLDataType dtype, RegName dst);
  /*! \brief Construct an allocate tensor with register instruction.
   *  \param shape_register The register containing the shape.
   *  \param dtype The dtype of the tensor.
   *  \param dst The destination register.
   *  \return The allocate tensor with register instruction.
   */
