...
class TypeSolver::Reporter : public TypeReporterNode {
 public:
  ...
  bool AssertEQ(const IndexExpr& lhs, const IndexExpr& rhs) final {
    ...
    if (lhs.same_as(Any()) || rhs.same_as(Any())) {
      // TODO: Assert Any!!! lhs == rhs
      LOG(WARNING) << "Assert " << lhs << " == " << rhs;
      return true;
    }
...
bool TypeSolver::Solve() {
  while (true) {
    ...
    if (num_resolved_rels_ != rel_nodes_.size()) {
      // Some type relation cannot be resolved
      break;
    }
    if (lifted_any_) {
      // Any lifted in last iteration, re-run type solve again
      lifted_any_ = false;
      num_resolved_rels_ = 0;
      for (auto rnode : rel_nodes_) {
        rnode->resolved = false;
        AddToQueue(rnode);
        rnode->inqueue = true;
      }
