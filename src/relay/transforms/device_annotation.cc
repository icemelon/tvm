  ...
  struct BottomUpVisitor : private ExprVisitor {
    ...
    void PropagateToChildren(const Expr& expr, int dev_type) {
      struct Propagator : public ExprVisitor {
        Propagator(int dev_ty, Map<Expr, Integer>* m)
            : dev_type(dev_ty), device_map(m) {}
      ...
      Propagator p(dev_type, &device_map);
      p.Visit(expr);
    }
    ...
    void VisitExpr_(const CallNode* call) final {
      ...
      if (const auto* copy = GetDeviceCopyNode(call)) {
        PropagateToChildren(GetRef<Call>(call), SourceDeviceType(copy));
        device_map.Set(GetRef<Call>(call), DestionationDeviceType(copy));
        if (root_device_copy == nullptr) {
          root_device_copy = copy;
        }
      }
    }
