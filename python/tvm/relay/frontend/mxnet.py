...
def _mx_foreach(inputs, attrs, subgraphs, dtype_info, mod):
    ...
    p = Prelude(mod)
    l, cons, nil = p.mod.get_type("List")
    ...
    with body_sb.else_scope():
        ...
        out_type = _infer_type(out).checked_type
    ...
    ret_type = [l(out_type)] + [_infer_type(s).checked_type for s in prev_states]
    ret_type = _ty.TupleType(ret_type)

    body = body_sb.get()
    loop_args = inputs + [num_iter, loop_iter, all_outs]
    func = _function.Function(loop_args, body, ret_type=ret_type)
    mod[loop] = func
