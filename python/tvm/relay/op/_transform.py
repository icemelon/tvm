...
def _arange_shape_func(attrs, inputs, outputs):
    ...
    start = ib.buffer_ptr(inputs[0])
    stop = ib.buffer_ptr(inputs[1])
    step = ib.buffer_ptr(inputs[2])
    out_buf = ib.buffer_ptr(outputs[0])
    out_buf[0] = _generic.cast(_intrin.ceil((
        _generic.cast(stop[0], "float32") - _generic.cast(start[0], "float32"))
        / _generic.cast(step[0], "float32")), "int64")
    body = ib.get()
    return body
