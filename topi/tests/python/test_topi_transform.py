...
from common import get_all_backend, get_schedule_injective, get_schedule_broadcast, get_schedule_elemwise
...
def verify_expand_dims(in_shape, out_shape, axis, num_newaxis):
    ...
    def check_device(device):
        ...
        with tvm.target.create(device):
            s = get_schedule_broadcast(device)(B)
...
def verify_concatenate(shapes, axis):
    ...
    def get_schedule_concatenate(target):
        schedule_map = {
        ...
        return get_schedule_injective(target)
    ...
    def check_device(device):
        ...
        print("Running on target: %s" % device)
        with tvm.target.create(device):
            s = get_schedule_concatenate(device)(out_tensor)
...
def verify_stack(shapes, axis):
    ...
    def check_device(device):
        ...
        print("Running on target: %s" % device)
        with tvm.target.create(device):
            s = get_schedule_broadcast(device)(out_tensor)
...
def verify_split(src_shape, indices_or_sections, axis):
    ...
    def check_device(device):
        ...
        print("Running on target: %s" % device)
        with tvm.target.create(device):
            s = get_schedule_injective(device)(tensor_l)
...
def verify_take(src_shape, indices_src, axis=None, mode="clip"):
    ...
    def check_device(device):
        ...
        print("Running on target: %s" % device)
        with tvm.target.create(device):
            s = get_schedule_injective(device)(out_tensor)
...
def verify_arange(start, stop, step):
    ...
    def check_device(device):
        ...
        print("Running on target: %s" % device)
        with tvm.target.create(device):
            s = get_schedule_injective(device)(A)
...
def verify_where(in_shape):
    ...
    def check_device(device):
        ...
        print("Running on target: %s" % device)
        with tvm.target.create(device):
            s = get_schedule_broadcast(device)(C)
...
def verify_one_hot(indices_shape, depth, on_value, off_value, axis, dtype):
    ...
    def check_device(device):
        ...
        print("Running on target: %s" % device)
        with tvm.target.create(device):
            s = get_schedule_injective(device)(one_hot_result)
...
def test_squeeze():
    ...
    for device in ['cuda', 'opencl']:
        ...
        if ctx.exist:
            with tvm.target.create(device):
                s = get_schedule_injective(device)(C)
